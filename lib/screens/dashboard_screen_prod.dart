...
import 'package:instapay_webtool_provider_test/utils/filtration_prod.dart';
...
import '../widgets/summary_card_clear.dart';
import '../widgets/summary_cards.dart';
import '../widgets/summary_transactions_dialog.dart';
import '../widgets/transaction_table.dart';

class DashboardScreenProd extends StatefulWidget {
  const DashboardScreenProd({super.key});

  @override
  _DashboardScreenStateProd createState() => _DashboardScreenStateProd();
}

class _DashboardScreenStateProd extends State<DashboardScreenProd> {
  final List<instapayModelProd> _users = [];
  List<instapayModelProd> _transactions = [];
  final _allTransactions = <instapayModelProd>[]; // All transactions
  List<instapayModelProd> _filteredTransactions = [];
  ...
  late InstapayFilterProd _instapayFilter;
  String? startDate;
  String? endDate;
  ...
  void initState() {
    ...
    _loadUsername();
    _instapayFilter = InstapayFilterProd(_transactions);
  }
  ...
  // Load transactions based on start and end date
  Future<void> _loadTransactions() async {
    setState(() {
      _isLoading = true;
    });

    // Set startDate to September 1, 2023, and endDate to current date
    DateTime now = DateTime.now();
    DateTime start = DateTime(2023, 9, 1); // September 1, 2023
    String startDate = start.toIso8601String(); // Current date as endDate
    String endDate = now.toIso8601String(); // Current date as endDate

    try {
      final transactions = await ApiService.fetchInstapayProd(
  ...
  Future<void> _loadCurrentTransactions() async {
    ...
    DateTime now = DateTime.now();
    String startDate = now.toIso8601String(); // Current date as endDate
    String endDate = now.toIso8601String(); // Current date as endDate
        ...
        cells: [
          ...
          DataCell(
            GestureDetector(
              ...
              child: Icon(Icons.info_outline, color: Color(0xff7b1113)), // Action icon
            ),
          ),
        ],
      );
